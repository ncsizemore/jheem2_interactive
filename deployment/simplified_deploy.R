#!/usr/bin/env Rscript
# simplified_deploy.R - Streamlined script to prepare and deploy to shinyapps.io
# This version assumes jheem2 is already installed and focuses only on file preparation
...
# ---------- STEP 6: Verify simulation directories ----------

cat("\n=== Verifying simulation directories ===\n")

# Get the simulation path from environment
simulation_path <- Sys.getenv("SIMULATION_PATH", "simulations/ryan-white")

# Make sure simulation directories exist
if (!dir.exists(paste0(simulation_path, "/prerun"))) {
  cat("WARNING: ", simulation_path, "/prerun directory does not exist!\n", sep = "")
}

if (!file.exists(paste0(simulation_path, "/base/C.12580_base.Rdata"))) {
  cat("WARNING: ", simulation_path, "/base/C.12580_base.Rdata file does not exist!\n", sep = "")
}
