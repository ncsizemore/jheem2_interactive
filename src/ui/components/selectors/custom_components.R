...
create_intervention_setting <- function(type, group_num, suffix, config, fixed_group = NULL) {
    ...
    base_id <- selector_config$id # Use ID generated by get_selector_config

    if (selector_config$type == "numeric") {
        ...
        tags$div(
            ...
            tags$div(
                ...
                numericInput(
                    ...
                    min = selector_config$min,
                    max = selector_config$max,
                    step = selector_config$step
                ),
                tags$div(
                    class = "input-error-message",
    ...
    } else if (selector_config$type == "compound") {
        # Compound input with checkbox and additional inputs
        # selector_config already has the correct ID
        tags$div(
            class = paste("intervention-component", type),
            tags$div(
                class = "input-validation-wrapper compound",
                # Pass the specific selector_config to create_compound_input
        ...
        warning(sprintf("Unknown component type: %s", selector_config$type))
        NULL
    }
}
...
create_subgroup_panel <- function(group_num, config, fixed_group = NULL) {
    # Validate config input
    if (missing(config) || !is.list(config)) {
        stop("create_subgroup_panel requires a valid 'config' object.")
    }
    # Get page type from config
    suffix <- config$page_type %||% "custom" # Default to custom if not set

    # Use the passed config object directly
    # config <- get_page_complete_config(suffix) # REMOVED
    ...
    tags$div(
        ...
        if (is.null(fixed_group) && !is.null(config$demographics)) {
            tags$div(
                ...
                # Pass the pre-loaded config down
                create_subgroup_characteristics(group_num, suffix, config = config)
            )
        },
        ...
        tags$div(
            ...
            lapply(names(config$interventions$components), function(type) {
                # Pass the pre-loaded config down
                create_intervention_setting(type, group_num, suffix, config = config, fixed_group = fixed_group)
            })
        )
    )
}
