#' @title Plotly Plotting Adapter
#'
#' This file contains temporary local implementations of the jheem2 plotly functions
#' that will be used until the updated versions are merged into the main jheem2 branch.
#'
#' @description
#' Contains:
...
#'
#' Both functions are based on forthcoming improvements to the jheem2 package.

#' @export
fixed_plot_simulations <- function(...,
                                   outcomes,
  ...
  # Create a fixed style manager that uses outcome instead of stratum
  # This avoids issues with empty stratum values
  fixed_style_manager <- create.style.manager(
    color.sim.by = 'outcome',  # Use outcome instead of stratum for coloring
    ...
    shape.sim.by = style.manager$shape.sim.by
  )
  
  fixed_execute_plotly_plot(prepared.plot.data,
    outcomes = outcomes,
    split.by = split.by,
    facet.by = facet.by,
    plot.which = plot.which,
    ...
    style.manager = fixed_style_manager,
    debug = debug
  )
}


fixed_execute_plotly_plot <- function(prepared.plot.data,
                                      outcomes = NULL,
                                      split.by = NULL,
                                      facet.by = NULL,
                                      plot.which = "sim.and.data",
  ...
  # Add robust error handling for stratum values
  # First check if stratum column exists and has valid values
  has_stratum <- "stratum" %in% names(df.truth)
  all_empty_stratum <- FALSE
  
  if (has_stratum) {
    # Check if all values are empty strings or NA
    all_empty_stratum <- all(is.na(df.truth$stratum) | df.truth$stratum == "")
  }
  
  # Handle color assignment with better logic
  if (style.manager$color.data.by == "stratum" && (has_stratum && all_empty_stratum)) {
    df.truth["color.and.shade.data.by"] <- df.truth["shade.data.by"]
  } else if (style.manager$shade.data.by == "stratum" && (has_stratum && all_empty_stratum)) {
  ...
  # Initialize y axis labels to prevent null references
  fig$y.axis.labels <- y.label
    ...
    if (!is.null(split.by)) {
      ...
      if (nrow(df.sim.groupids.many.members) > 0) {
        ...
        if (is.null(facet.by)) {
          ...
          # Set y-axis labels for all facets
      ...
      if (nrow(df.sim.groupids.many.members) > 0) {
        ...
        if (is.null(facet.by)) {
          ...
          # Set y-axis labels for each outcome
          if (is.null(fig$y.axis.labels)) {
          fig$y.axis.labels <- c()
          }
          ...
          facet.categories <- unique(df.sim.groupids.many.members[[facet.by]])
          figure.count <- length(facet.categories)
          # Set y-axis labels for faceting
          if (is.null(fig$y.axis.labels)) {
            fig$y.axis.labels <- c()
          }
  ...
  if (figure.count > 1) {
    ...
    for (i in 1:figure.count) {
      ...
      y_axis_label <- if ("y.axis.labels" %in% names(fig) && length(fig$y.axis.labels) >= i) {
        fig$y.axis.labels[i]
      } else {
        y.label  # Use default y label if specific one not available
      }
      
      fig$layout[[paste0("yaxis", i)]] <- list(title = y_axis_label, domain = c(y_bottom, y_top), anchor = paste0("x", i))
      # Make sure we have a valid facet category for this index
      facet_label <- if (!is.null(facet.categories) && length(facet.categories) >= i) {
        facet.categories[i]
      } else {
        paste("Facet", i)  # Fallback label if categories not available
      }
    ...
    y_axis_label <- if ("y.axis.labels" %in% names(fig) && length(fig$y.axis.labels) >= 1) {
      fig$y.axis.labels[1]
    } else {
      y.label  # Use default y label if specific one not available
    }
    
    fig$layout[["yaxis"]] <- list(title = y_axis_label, anchor = "x1")
  }
  # browser()
