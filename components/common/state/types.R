# components/common/state/types.R
...
validate_control_state <- function(state) {
    ...
    missing <- setdiff(required, names(state))
    if (length(missing) > 0) {
        stop(sprintf("Missing required control state fields: %s",
                     paste(missing, collapse = ", ")))
    }
...
validate_panel_state <- function(state) {
    ...
    missing <- setdiff(required, names(state))
    if (length(missing) > 0) {
        stop(sprintf("Missing required panel state fields: %s",
                     paste(missing, collapse = ", ")))
    }
