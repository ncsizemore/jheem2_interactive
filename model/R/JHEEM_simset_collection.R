    ...
    public = list(
                       ...
                       stop.for.errors = F)
        {
            ...
            if (check.consistency &&
                (!is.list(mappings) || (length(mappings)>0 && is.null(names(mappings))) || any(sapply(mappings, function(x) {!R6::is.R6(x) || !is(x, 'ontology.mapping')})) || (!is.null(names(mappings)) && !all(names(mappings) %in% outcomes))))
                stop(paste0(error.prefix, "'mappings' must be a list of zero or more ontology mappings with outcomes as names"))
